...
**управление роботом с помощью регулятора, созданного на основе функции Ляпунова**
...
длительные попытки починить соединение на старом KFS1, сломанный адаптер wifi	

git clone, передача файлов через sftp, ssh
работа с сетью(nmap, networkscanner)
запуск решения на роботе и на сервере(создание виртуальных окружений и настройка систем, pip, запуск python скриптов)

отладка решения сокомандников с помощью wsl2(Ubuntu 20.04.06)
